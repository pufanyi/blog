...
因此，**loss 的负梯度 $-\nabla_\theta\ell_\theta(\tau)$ 是 policy gradient 的无偏估计**，loss 的数值本身并不估计负的期望奖励。Spinning Up [@SpinningUp2018] 强调，这个无偏保证是在采样所用的策略参数处成立的；更新参数后，一直复用旧 batch 最小化它，并没有同样的保证。
...
Schulman 等人的 GAE [@schulman2015high]。

我们考虑 advantage 在 policy gradient 中的这一项：

$$
\mathbb E\left[\Psi_t\nabla_\theta\log\pi_\theta(a_t\mid s_t)\right].
$$

其中如果 $\Psi_t=\mathcal A^{\pi_\theta,\gamma}(s_t,a_t)$ 的话，是对的。那我们有没有啥其他的 $\Psi_t$ 也能给出相同的期望呢？我们把满足
...
的估计量叫做 $\gamma$-just 的。它说的是这个 discounted advantage 的梯度项，不能因此忽略前面讨论的目标与时间权重约定。

使用真实的 value function 时，

$$
r(s_t,a_t)+\gamma\mathcal V^{\pi_\theta,\gamma}(s_{t+1})
-\mathcal V^{\pi_\theta,\gamma}(s_t)
$$

是 $\gamma$-just 的。进一步的推导待续。
