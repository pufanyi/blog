...
<span id="denoising-as-an-mdp" />
<span id="from-a-deterministic-flow-to-a-stochastic-policy" />
<span id="group-rewards-and-policy-updates" />
<span id="flow-grpo-and-dancegrpo" />
<span id="sampling-details-that-change-the-rl-objective" />

把 denoising 的每一步看成一次 action，就可以用最终图片或视频的 reward 更新生成模型。[Diffusion 中的 RL for diffusion models](/blog/ml/ml-revisit/diffusion#rl-for-diffusion-models) 接着采样过程，介绍这个 MDP、group-relative policy updates，以及 Flow-GRPO 和 DanceGRPO。

<span id="coefficients-preserving-sampling" />

随机 rollout 中已有 noise 与新 noise 的分配，以及它对 RL 训练的影响，见该章的 [Coefficients-Preserving Sampling（CPS）](/blog/ml/ml-revisit/diffusion#coefficients-preserving-sampling)。
