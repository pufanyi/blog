...
CPS 的采样原理见 [Coefficients-Preserving Sampling](/blog/ml/ml-revisit/diffusion#coefficients-preserving-sampling)。它把下一步的 noise 份额分给 retained noise 与 fresh noise，可以替换这里的随机 sampler。对 $t_k\to t_{k+1}$，它的 mean 和 standard deviation 是
...
只要 $\nu_k>0$，它仍然定义 Gaussian transition。但应区分 **CPS sampler** 和 Flow-CPS 论文 [@wang2025cps] 为稳定训练额外修改的 surrogate；换了采样公式，并不意味着代码中的任何 `log_prob` 都自动是它的严格 density。
