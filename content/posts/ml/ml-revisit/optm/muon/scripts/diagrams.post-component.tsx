...
function MathLabel({ x, y, width = 80, tex }: {
  x: number;
  y: number;
  width?: number;
  tex: string;
...
function Diagram({ id, title, description, height, caption, children }: {
  ...
  return (
    <figure id={id} className="muon-figure">
      <div className="muon-scroll" role="region" aria-label={`${title}，窄屏可横向滚动`} tabIndex={0}>
        <svg className="muon-diagram" xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 660 ${height}`} width="660" height={height} role="img" aria-labelledby={`${id}-title ${id}-desc`}>
...
function MuonSpectrum() {
  const panels = [
    ...
    { title: 'Ideal polar factor', values: SINGULAR_VALUES.map(() => 1), note: 'All nonzero values become 1' },
    { title: 'Five quintic steps', values: finalValues, note: 'Similar scale, not exactly 1' },
  ];
  const baseline = 230;
  const scale = 125;
  return (
    <Diagram id="muon-spectrum" title="整体归一化、理想 polar 变换与五步近似的奇异值" height={314}
      ...
      {panels.map((panel, panelIndex) => {
        ...
        return (
          <g key={panel.title}>
            <text x={left + 110} y="30" className="muon-heading">{panel.title}</text>
            <path d={`M ${left + 24} ${baseline - scale} H ${left + 207}`} className="muon-reference" />
            <text x={left + 15} y={baseline - scale + 4} className="muon-note">1</text>
            ...
            {panel.values.map((value, index) => {
              ...
              return (
                <g key={index} className={`muon-${TONES[index]}`}>
                  <rect x={center - 15} y={top} width="30" height={baseline - top} rx="3" className="muon-bar" />
                  <text x={center} y={top - 10} className="muon-value">{value.toFixed(3)}</text>
                  <MathLabel x={center - 26} y={236} width={52} tex={`\\sigma_${index + 1}`} />
                </g>
              );
            })}
            <text x={left + 110} y="295" className="muon-note">{panel.note}</text>
          </g>
        );
      })}
    </Diagram>
...
function MuonIteration() {
  ...
  return (
    <>
      <Diagram id="muon-iteration" title="五次多项式如何改变每个奇异值" height={352}
        ...
        <text x="330" y="27" className="muon-heading">One update · five polynomial iterations</text>
        {[0, 0.5, 1, 1.5].map(value => (
          <g key={value}>
            <path d={`M 72 ${plotY(value)} H 592`} className={value === 1 ? 'muon-reference' : 'muon-grid'} />
            <text x="45" y={plotY(value) + 4} className="muon-note">{value.toFixed(1)}</text>
          </g>
        ))}
        <path d="M 72 48 V 258 H 592" className="muon-axis" />
        {iterates.map((_, step) => <text key={step} x={plotX(step)} y="283" className="muon-note">{step}</text>)}
        ...
        {SINGULAR_VALUES.map((_, direction) => (
          <g key={direction} className={`muon-${TONES[direction]} muon-series-${direction}`}>
            <path d={iterates.map((values, step) => `${step === 0 ? 'M' : 'L'} ${plotX(step)} ${plotY(values[direction]!)}`).join(' ')} className="muon-curve" />
            {iterates.map((values, step) => <circle key={step} cx={plotX(step)} cy={plotY(values[direction]!)} r="3.5" className="muon-point" />)}
            <path d={`M ${149 + direction * 155} 327 h 32`} className="muon-curve" />
            <MathLabel x={184 + direction * 155} y={310} width={58} tex={`\\sigma_${direction + 1}`} />
          </g>
        ))}
      </Diagram>
      <details>
        <summary>示例每步的奇异值</summary>
        <p>从对角矩阵的奇异值 {SINGULAR_VALUES.join('、')} 开始，先除以 Frobenius norm 加 {EPSILON}，再使用正文的五次多项式。表格保留三位小数，各列跟踪原始方向。</p>
        <table>
          <thead><tr><th>迭代次数</th>{SINGULAR_VALUES.map((_, index) => <th key={index}>{`\\(\\sigma_${index + 1}\\)`}</th>)}</tr></thead>
          <tbody>{iterates.map((values, step) => <tr key={step}><td>{step}</td>{values.map((value, index) => <td key={index}>{value.toFixed(3)}</td>)}</tr>)}</tbody>
        </table>
      </details>
    </>
  );
