...
function SiameseBranches() {
  ...
  return (
    <Figure
      ...
      caption="BYOL 的单方向示意。实线是前向数据流，虚线是 EMA；target 不通过 loss 反传，但随 EMA 持续更新。"
      height={268}
    >
      ...
      <Label x={249} y={124} note>
        EMA
      </Label>
      <Label x={330} y={241}>
        SimSiam：共享 encoder / projector 参数，没有独立 EMA teacher
      </Label>
    </Figure>
  );
}
...
function CollapseGeometry() {
  ...
  return (
    <Figure
      ...
      caption="同一套坐标比例。VICReg 的 variance 项针对左图，covariance 项还会惩罚中图的重复方向；右图只是几何示意。"
      height={247}
    >
      {panels.map((panel, i) => {
        const cx = 110 + i * 220;
