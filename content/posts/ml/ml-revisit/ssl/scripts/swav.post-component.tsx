...
function SwappedAssignments() {
  ...
  return (
    <Figure
      id={id}
      title="SwAV 的 swapped prediction"
      ...
      caption="每条跨行箭头表示来自另一 view 的监督。Assignment 由整个 batch 的 scores 联合计算，作为固定目标。"
      height={292}
    >
      {[0, 1].map((i) => {
        const y = 45 + i * 150;
        return (
          <g key={i}>
            ...
            <Box
              ...
              y={y}
              width={154}
              label={`View ${i + 1} 的 assignment`}
              sub="Balanced；stop-gradient"
              tone="teal"
            />
            ...
            <Arrow
              id={id}
              d={`M 195 ${i === 0 ? y : y + 52} V ${i === 0 ? 16 : 275} H 570 V ${i === 0 ? y - 2 : y + 54}`}
            />
          </g>
        );
      })}
      ...
      <Label x={163} y={163} note>
        用对方的 assignment 训练自己的预测
      </Label>
    </Figure>
  );
}
