...
export function Figure({
  ...
  return (
    <figure className="ssl-figure">
      <div
        className="ssl-scroll"
        tabIndex={0}
        role="region"
        aria-label={`${title}；窄屏可横向滚动`}
      >
        <svg
          className="ssl-diagram"
          width="660"
          height={height}
      ...
      <p className="ssl-scroll-hint" data-agent-omit="true">
        窄屏可横向滚动查看完整示意图
      </p>
...
export function Label({
  ...
  return (
    <text x={x} y={y} className={note ? 'ssl-note' : 'ssl-label'}>
      {children}
    </text>
  );
}
...
export function Box({
  ...
  return (
    <g className={`ssl-${tone}`}>
      ...
      <Label x={x + width / 2} y={y + height / 2 + (sub ? -4 : 4)}>
        {label}
      </Label>
      {sub && (
        <Label x={x + width / 2} y={y + height / 2 + 15} note>
          {sub}
        </Label>
      )}
    </g>
