...
function SSLWorkflow() {
  ...
  return <Figure id={id} title="预训练学 encoder，下游任务复用表征"
    ...
    caption="先分清两个空间：训练 head 服务于自监督任务，下游任务通常复用 encoder 的表征。" height={322}>
    <Label x={330} y={25}>预训练：从数据自身构造监督</Label>
    ...
    <Label x={430} y={185} note>点线：loss 经 head 反传，更新 encoder</Label>
    <path d="M 16 203 H 644" className="ssl-divider" />
    <Label x={95} y={233}>下游任务</Label>
    <ImageTile x={30} y={251} size={44} />
    <Box x={215} y={246} width={115} label="Encoder" sub="复用预训练参数" />
    <Vector x={361} y={256} />
    <Box x={432} y={246} width={216} label="分类 / 检索 / 分割" sub="按任务选择特征与评估方式" tone="teal" />
...
export function simclrCandidate(anchor: number, candidate: number) {
  return anchor === candidate ? 'self' : SIMCLR_VIEWS[anchor] === SIMCLR_VIEWS[candidate] ? 'positive' : 'negative';
}

function SimCLRBatch() {
  const id = 'ssl-simclr-batch';
  return <Figure id={id} title="SimCLR：两个 view 如何变成 batch 内的分类任务"
    ...
    caption="例子取 3 张图：每行保留 5 个候选，其中 1 个 positive、4 个 negatives。带 × 的自身不进入分母。" height={372}>
    <Label x={154} y={27}>同一张图的两份增强</Label>
    ...
    <Vector x={310} y={68} /><Vector x={310} y={151} tone="teal" />
    <Arrow id={id} d="M 77 82 H 107 M 77 165 H 107 M 276 82 H 304 M 276 165 H 304" />
    <MathLabel x={304} y={96} width={48} tex="A_1" /><MathLabel x={304} y={179} width={48} tex="A_2" />
    <Label x={167} y={238} note>另外两张图也各自产生两份 view</Label>
    <Label x={535} y={27}>与全部 views 比相似度</Label>
    {VIEWS.map((name, i) => <g key={name}>
      ...
      {VIEWS.map((_, j) => {
        ...
        return <g key={j} data-candidate-role={role}>
          <rect x={436 + j * 34} y={63 + i * 34} width="30" height="30"
            className={role === 'self' ? 'ssl-excluded' : role === 'positive' ? 'ssl-positive' : 'ssl-cell'} />
          {role !== 'negative' && <Label x={451 + j * 34} y={83 + i * 34} note>{role === 'self' ? '×' : '+'}</Label>}
        </g>;
      })}
    </g>)}
    <rect x="432" y="59" width="208" height="38" rx="3" className="ssl-highlight" />
    <Label x={330} y={302}>选第一行作 anchor：提高同图另一 view 相对于其余候选的概率</Label>
    <Label x={330} y={330} note>棕框：当前 anchor 行　　绿色 +：positive　　空白格：negatives</Label>
    <Label x={330} y={350} note>矩阵表示候选关系，不是训练得到的相似度数值</Label>
  </Figure>;
}
