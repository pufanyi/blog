...
function ContrastiveDictionary() {
  ...
  return (
    <Figure
      id={id}
      title="SimCLR 与 MoCo v1/v2 的候选集合"
      ...
      caption="实线表示数据流，虚线表示 EMA 参数更新。MoCo v3 不使用图中 v1/v2 的历史 queue。"
      height={326}
    >
      <Label x={90} y={28}>
        SimCLR
      ...
      <Box
        ...
        width={212}
        label="当前 batch 的候选"
        sub="一个 positive，其余 negatives"
        tone="teal"
      />
      <Arrow id={id} d="M 160 73 H 205" />
      <Arrow id={id} d="M 381 73 H 426" />
      <Label x={330} y={124} note>
        每个 view 都轮流成为 anchor；从候选中排除自身
      </Label>
      <Label x={90} y={161}>
        MoCo v1/v2
      </Label>
      ...
      <Label x={183} y={249} note>
        EMA
      </Label>
      <Label x={364} y={306} note>
        新 keys 入队，最旧 keys 出队
      </Label>
    </Figure>
  );
}
