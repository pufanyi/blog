...
function MaskedTargets() {
  ...
  return (
    <Figure
      ...
      caption="上图以 16 个 patches 保留 4 个示意 MAE；下图只比较 BEiT 的计算路径，并不共用上图的 mask ratio。"
      height={330}
    >
      <Label x={80} y={25}>
        MAE：只编码可见部分
      ...
      {PATCHES.map((p) => (
        <rect
          ...
          width="22"
          height="22"
          className={p.visible ? 'ssl-visible' : 'ssl-masked'}
        />
      ))}
      ...
      <Label x={390} y={306} note>
        完整原图 → 固定 tokenizer → target IDs
      </Label>
