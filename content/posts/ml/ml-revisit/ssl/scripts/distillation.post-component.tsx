...
function Distribution({ x, y, values }: { x: number; y: number; values: number[] }) {
  return (
    <g className="ssl-teal" aria-hidden="true">
      {values.map((p, i) => (
        <rect
          key={i}
          x={x + i * 17}
...
function DINOBranches() {
  ...
  return (
    <Figure
      id={id}
      title="DINO：local 与 global views 向 EMA teacher 学习"
      ...
      caption="实线：前向；长虚线：EMA；点线：梯度。两组小柱仅示意分布，输出槽位没有预先指定的类别含义。"
      height={407}
    >
      <Label x={76} y={25}>
        Global + local crops
      </Label>
      <ImageTile x={20} y={43} size={49} />
      <ImageTile x={84} y={43} size={49} />
      {[25, 65, 105].map((x) => (
        <ImageTile key={x} x={x} y={104} size={27} local />
      ))}
      ...
      <Label x={421} y={184} note>
        排除 student 与 teacher
      </Label>
      <Label x={421} y={202} note>
        看同一份 crop 的配对
      </Label>
      <Label x={76} y={252}>
        仅 global crops
      </Label>
      <ImageTile x={20} y={271} size={49} />
      <ImageTile x={84} y={271} size={49} />
      <Box
        x={183}
        y={269}
        width={164}
      ...
      <Box
        ...
        y={269}
        width={149}
        label="Center + sharpen"
        sub="Teacher temperature"
        tone="teal"
      />
      ...
      <Label x={603} y={248} note>
        Stop-grad
      </Label>
      <Arrow id={id} d="M 265 117 V 267" update />
      <Label x={306} y={230} note>
        参数 EMA
      </Label>
      ...
      <Label x={240} y={377} note>
        Teacher batch logits 的均值
      </Label>
    </Figure>
  );
}
...
export function patchGram(features: number[][]): number[][] {
  ...
  return normalized.map((a) =>
    normalized.map((b) => a.reduce((sum, value, i) => sum + value * b[i]!, 0)),
  );
}
...
function Gram({ x, y, features }: { x: number; y: number; features: number[][] }) {
  return (
    <g className="ssl-teal" aria-hidden="true">
      {patchGram(features).flatMap((row, i) =>
        row.map((value, j) => (
          <g key={`${i}-${j}`}>
            ...
            <rect
              ...
              height="19"
              fill="currentColor"
              opacity={0.08 + Math.abs(value) * 0.55}
            />
          </g>
        )),
      )}
...
function PatchDistillation() {
  ...
  return (
    <Figure
      id={id}
      title="从全局分布到 patch 与 patch 的关系"
      ...
      caption="每行突出一个监督尺度。前两行的 teacher 由 EMA 更新；第三行使用稳定参考，Gram 方格仅作示意。"
      height={400}
    >
      <Label x={330} y={25}>
        图像级：不同 crop 的全局判断一致
      ...
      <Label x={330} y={385} note>
        比较位置之间的相似性，不是不同特征维度的 covariance
      </Label>
    </Figure>
  );
}
