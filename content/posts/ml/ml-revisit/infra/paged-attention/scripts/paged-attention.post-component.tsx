      ...
      {Array.from({ length: BLOCK_SIZE }, (_, index) => (
        <g key={index}>
          <rect
            ...
            height="30"
            rx="3"
            className={index < tokens.length ? 'paged-cell' : 'paged-empty'}
          />
          {tokens[index] !== undefined && (
            <text x={x + index * cell + (cell - 2) / 2} y={y + 20} className="paged-token">
              {tokens[index]}
            </text>
          )}
        </g>
...
function PagedKvLayout() {
  const logicalBlocks = REQUEST_A.blocks.map((physical, logical) => ({
    ...
    logical,
    tokens: Array.from(
      { length: Math.min(BLOCK_SIZE, REQUEST_A.tokens - logical * BLOCK_SIZE) },
      (_, offset) => String(logical * BLOCK_SIZE + offset),
    ),
  }));
  ...
  return (
    <Diagram
      id="paged-kv-layout"
      title="Logical KV → block table → physical pool"
      height={340}
    ...
    >
      <text
        x="340"
        y="24"
        className="paged-heading"
      >{`Request A · ${REQUEST_A.tokens} cached tokens`}</text>
      {logicalBlocks.map(({ logical, physical, tokens }) => {
        ...
        return (
          <g key={logical}>
            ...
            <path
              d={`M ${center} 154 C ${center} 189, ${destination} 182, ${destination} 218`}
              className="paged-line"
              markerEnd="url(#paged-kv-layout-arrow)"
            />
          </g>
        );
      })}
      {pool.map(({ name, tokens, tone }, physical) => (
        <g key={physical}>
          ...
          <text x={76 + physical * 132} y="302" className="paged-note">
            {name}
          </text>
        </g>
      ))}
      <text x="340" y="330" className="paged-note">
...
function WasteBracket({ x, width, above }: { x: number; width: number; above: boolean }) {
  ...
  return (
    <path
      d={`M ${x} ${y} Q ${x} ${y + bend} ${x + 7} ${y + bend} H ${center - 7} Q ${center} ${y + bend} ${center} ${y + 2 * bend} Q ${center} ${y + bend} ${center + 7} ${y + bend} H ${end - 7} Q ${end} ${y + bend} ${end} ${y}`}
      className="paged-line paged-waste-bracket"
    />
  );
}

function PagedKvWaste() {
  const count = (kind: (typeof NAIVE_KV_REGIONS)[number]['kind']) =>
    NAIVE_KV_REGIONS.find((region) => region.kind === kind)!.tokens.length;
  ...
  return (
    <Diagram
      id="paged-kv-waste"
      title="KV cache memory: prompt, generated tokens, and three forms of waste"
      height={202}
    ...
    >
      {NAIVE_KV_REGIONS.map((region, index) => {
        const start = NAIVE_KV_REGIONS.slice(0, index).reduce(
          (sum, previous) => sum + previous.tokens.length,
          0,
        );
          ...
          <g key={region.kind} className={`paged-${region.tone}`}>
            {region.tokens.map((token, offset) => (
              <g key={offset}>
                <rect
                  ...
                  width={cell}
                  height="40"
                  className={`paged-cell paged-waste-cell paged-waste-${region.kind}`}
                />
                <text
                  x={x + (offset + 0.5) * cell}
                  y="111"
                  className="paged-token paged-waste-token"
                >
                  {token}
                </text>
              </g>
            ...
            {labels.map((label, line) => (
              <text
                key={line}
                x={x + width / 2}
                y={(region.above ? 42 : 166) + line * 17}
                className="paged-note paged-waste-annotation"
              >
                {label}
              </text>
            ))}
      ...
      <path
        d={`M ${currentX} 152 V 131`}
        className="paged-line"
        markerEnd="url(#paged-kv-waste-arrow)"
      />
      <text x={currentX} y="166" className="paged-note">
        Request A
      </text>
      <text x={currentX} y="183" className="paged-note">
        current step
      </text>
    </Diagram>
  );
...
function PagedKvSharing() {
  return (
    <Diagram
      id="paged-kv-sharing"
      title="Shared prefix · copy only the partial tail"
      height={290}
    ...
    >
      <text x="340" y="26" className="paged-heading">
        After A and B append different tokens
      </text>
      <text x="136" y="106" className="paged-label">
        P3 · shared prefix
      </text>
      <Cells x={65} y={120} tokens={PREFIX} cell={36} />
      <text x="136" y="178" className="paged-note">
        refcount = 2
      </text>
      {BRANCHES.map(({ name, token, block, tone, copied }, index) => {
        ...
        return (
          <g key={name}>
            <path
              d={`M 216 ${130 + index * 10} C 294 ${130 + index * 10}, 306 ${y + 15}, 400 ${y + 15}`}
              className={`paged-line${copied ? '' : ' paged-dashed'}`}
              markerEnd="url(#paged-kv-sharing-arrow)"
            />
            ...
            <text
              x="527"
              y={y + 54}
              className="paged-note"
            >{`${copied ? 'Copied tail' : 'Original tail'} · refcount = 1`}</text>
          </g>
        );
      })}
    </Diagram>
...
function PagedContinuousBatching() {
  return (
    <Diagram
      id="paged-batching"
      title="Continuous batching · refill at iteration boundaries"
      height={224}
    ...
    >
      {(['Static batch', 'Continuous batch'] as const).map((label, panel) => {
        ...
        const finish = Math.max(
          ...DECODE_REQUESTS.filter(({ ready }) => ready === 0).map(({ steps }) => steps),
        );
        const cStart = panel === 0 ? finish : c.ready;
        return (
          <g key={label}>
            <text x={origin + 150} y="28" className="paged-heading">
              {label}
            </text>
            {Array.from({ length: finish }, (_, iteration) => (
              <g key={iteration}>
                <text
                  x={origin + 56 + iteration * 98}
                  y="57"
                  className="paged-note"
                >{`Step ${iteration + 1}`}</text>
                {[0, 1].map((slot) => {
                  const first = DECODE_REQUESTS[slot]!;
                  const request =
                    iteration < first.steps
                      ? first
                      : slot === 1 && iteration >= cStart && iteration < cStart + c.steps
                        ? c
                        : null;
                  return (
                    <g key={slot} className={`paged-${request?.tone ?? 'neutral'}`}>
                      <rect
                        x={origin + 14 + iteration * 98}
                        y={72 + slot * 52}
                        width="84"
                        height="38"
                        rx="5"
                        className={request ? 'paged-cell' : 'paged-empty'}
                      />
                      <text
                        x={origin + 56 + iteration * 98}
                        y={97 + slot * 52}
                        className="paged-token"
                      >
                        {request?.name ?? 'idle'}
                      </text>
                    </g>
            ...
            <text x={origin + 150} y="196" className="paged-note">
              {panel === 0 ? 'C waits until this batch finishes' : 'B finishes → C joins at step 2'}
            </text>
          </g>
        );
      })}
