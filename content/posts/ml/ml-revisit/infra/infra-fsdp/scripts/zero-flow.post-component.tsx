    ...
    <g className={`infra-zero-flow-tensors infra-zero-${tensor}`}>
      {RANKS.map((rank) => (
        <g key={rank}>
          ...
          {reduced && (
            <rect
              x={gpuX(rank) + rank * CELL}
              y={y}
              width={CELL}
              height={BAR_HEIGHT}
              className="infra-zero-flow-reduced"
            />
          )}
        </g>
      ))}
    ...
    <g className={`infra-zero-flow-exchange infra-zero-${tensor}`}>
      {zeroTransfers(operation).map(({ source, destination, shard }) => {
        ...
        const destinationPort =
          operation === 'reduce-scatter' ? (source + 0.5) / RANKS.length : 0.5;
        return (
          <path
  ...
  return (
    <g className="infra-zero-flow-local">
      {RANKS.map((rank) => (
        <path
          key={rank}
          d={`M ${gpuX(rank) + BAR_WIDTH / 2} ${from} V ${to}`}
          markerEnd={`url(#zero-flow-${stage}-local-arrow)`}
        />
      ))}
    </g>
  );
...
function Operation({ y, title }: { y: number; title: string }) {
  return (
    <g>
      ...
      <text x="126" y={y + 20} className="infra-zero-flow-action">
        {title}
      </text>
    </g>
  );
  ...
  return (
    ...
    >
      ...
      {RANKS.map((rank) => (
        <text
          key={rank}
          x={gpuX(rank) + BAR_WIDTH / 2}
          y="20"
          className="infra-zero-flow-rank"
        >{`GPU ${rank}`}</text>
      ))}
      <TensorRow y={32} tensor="parameters" layout={parameterLayout} />
      <TensorRow y={45} tensor="optimizer" layout="sharded" />
      <text x="126" y="73" className="infra-zero-flow-note">
        迭代开始 · 各卡持有的状态
      </text>

      <rect x="8" y="88" width="232" height="230" rx="5" className="infra-zero-flow-phase" />
      <text x="20" y="109" className="infra-zero-flow-phase-title">
        FORWARD · 层 1 → L
      </text>
      <text
        x="126"
        y="134"
        className={`infra-zero-flow-label${parametersSharded ? ' infra-zero-parameters' : ''}`}
      >
        {parametersSharded ? 'All-Gather · 本层参数' : '完整参数已在本卡 · 无通信'}
      </text>
      <TensorRow y={148} tensor="parameters" layout={parameterLayout} />
      {parametersSharded ? (
        <Exchange stage={stage} operation="all-gather" from={148} to={210} />
      ) : (
        <LocalFlow stage={stage} from={160} to={205} />
      )}
      ...
      <text x="126" y="306" className="infra-zero-flow-note">
        {parametersSharded ? '用完释放完整参数 → 下一层' : '完整参数常驻 → 下一层'}
      </text>

      <rect x="8" y="334" width="232" height="312" rx="5" className="infra-zero-flow-phase" />
      <text x="20" y="355" className="infra-zero-flow-phase-title">
        BACKWARD · 层 L → 1
      </text>
      <text
        x="126"
      ...
      {parametersSharded ? (
        <Exchange stage={stage} operation="all-gather" from={394} to={446} />
      ) : (
        <LocalFlow stage={stage} from={406} to={441} />
      )}
      <TensorRow y={446} tensor="parameters" layout="replicated" />
      <Operation y={462} title="Backward" />
      <text x="126" y="512" className="infra-zero-flow-label infra-zero-gradients">
        Reduce-Scatter · 平均梯度
      </text>
      ...
      <text x="126" y="611" className="infra-zero-flow-note">
        {stage === 1 ? '保留完整梯度缓冲区' : '逐 bucket 归约 · 只保留本卡梯度'}
      </text>
      <text x="126" y="631" className="infra-zero-flow-note">
        {parametersSharded
          ? '释放本层完整参数 → 上一层'
          : stage === 1
            ? '描边：本卡负责的已归约分片'
      ...
      <text x="20" y="683" className="infra-zero-flow-phase-title">
        OPTIMIZER STEP
      </text>
      ...
      {parametersSharded ? (
        <LocalFlow stage={stage} from={768} to={811} />
      ) : (
        <Exchange stage={stage} operation="all-gather" from={756} to={816} />
      )}
      <TensorRow y={816} tensor="parameters" layout={parameterLayout} />
      <text x="126" y="844" className="infra-zero-flow-note">
        {parametersSharded ? '下一步前向时再按层聚合' : '完整模型就绪 → 下一步前向'}
      </text>
    </svg>
  );
}
...
function ZeroFlowComparison() {
  return (
    <figure id="zero-flow" className="infra-zero-flow-figure">
      <div
        className="infra-zero-flow-scroll"
        role="region"
        aria-label="ZeRO 三种方法的训练流程对比，可横向滚动"
        tabIndex={0}
      >
        <div className="infra-zero-flow-grid">
          {METHODS.map(({ stage, partition }) => (
            <div key={stage} className="infra-zero-flow-panel">
              <div className="infra-zero-flow-heading">
                <strong>{`ZeRO-${stage}`}</strong>
                <span>{partition}</span>
              </div>
      ...
      <div className="infra-zero-legend">
        <span className="infra-zero-legend-item infra-zero-parameters">
          <span className="infra-zero-swatch" aria-hidden="true" />
          <span>Parameters · 实线传输</span>
        </span>
        <span className="infra-zero-legend-item infra-zero-gradients">
          <span className="infra-zero-swatch" aria-hidden="true" />
          <span>Gradients · 虚线归约</span>
        </span>
        <span className="infra-zero-legend-item infra-zero-optimizer">
          <span className="infra-zero-swatch" aria-hidden="true" />
