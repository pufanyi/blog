...
假设把这组 score 分成前半段和后半段。计算前半段的任意一个权重时，分母仍是**全部 score 的指数和**，也包含尚未读到的后半段。因此，不能只对前半段做一次 softmax，就把结果当作最终权重。

例如，四个 score 的指数值依次为 $1,2,4,8$。只对前半段做 softmax，会得到 $1/3,2/3$；但把后半段也算进去，分母变成 $1+2+4+8=15$，前两个位置的权重应当是 $1/15,2/15$。
