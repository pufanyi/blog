...
const posts = Array.from({ length: 23 }, (_, index) => ({
  slug: `post-${index}`, title: `Post ${index}`, date: '2026-09-06', description: 'A test post',
}));
...
describe('blog pagination', () => {
  it('partitions every post exactly once for different configured page sizes', () => {
    for (const size of [1, 4, 10, 23, 100]) {
      const pages = Array.from({ length: blogPageCount(posts.length, size) }, (_, index) => paginatePosts(posts, String(index + 1), size)!);
    ...
    expect(paginatePosts(posts, '2', 10)).toMatchObject({ number: 2, totalPages: 3, start: 11, end: 20 });
  ...
  it('uses /blog for the first page and handles an empty archive', () => {
    ...
    expect(paginatePosts([], null, 10)).toEqual({ number: 1, totalPages: 1, totalPosts: 0, start: 0, end: 0, posts: [] });
    expect(paginatePosts([], '2', 10)).toBeNull();
  });

  it('rejects malformed and out-of-range page numbers', () => {
    for (const page of ['', '0', '-1', '1.5', '02', '+2', '2abc', '1e2', '999', '9007199254740993']) {
