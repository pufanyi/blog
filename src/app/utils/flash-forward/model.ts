...
export interface Tile { index: number; start: number; end: number }
...
export function partition(length: number, size: number): Tile[] {
  ...
  return Array.from({ length: Math.ceil(length / size) }, (_, index) => ({
    index, start: index * size, end: Math.min((index + 1) * size, length),
  }));
}
