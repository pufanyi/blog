...
describe('blog directories', () => {
  it('preserves archive order when same-day posts move into different categories', () => {
    const posts = [
      post('a/zebra', '2026-01-01'),
      post('z/apple', '2026-01-01'),
      post('b/new', '2026-01-02'),
    ];
    expect(posts.sort(comparePostsByPublication).map((post) => post.slug)).toEqual([
      'b/new',
      'z/apple',
      'a/zebra',
    ]);
  ...
  it('lists immediate children and propagates publication dates while ignoring later updates', () => {
    ...
    expect(
      root.entries.map((entry) => [entry.kind, entry.slug, entry.date, entry.postCount]),
    ).toEqual([
    ...
    expect(
      directories
        .find((directory) => directory.slug === 'oi/codeforces')!
        .entries.map((entry) => entry.slug),
    ).toEqual(['oi/codeforces/new', 'oi/codeforces/old']);
    expect(directories.find((directory) => directory.slug === 'oi/notes/deep')!.date).toBe(
      '2026-09-09',
    );
  ...
  it('keeps a contents root for empty and flat collections and computes directory navigation', () => {
    expect(buildBlogDirectories([])).toEqual([
      { slug: '', name: 'Contents', postCount: 0, entries: [] },
    ]);
    const directories = buildBlogDirectories([post('flat', '2026-01-01')]);
    expect(directories).toHaveLength(1);
    expect(directories[0].entries).toEqual([
      { kind: 'post', slug: 'flat', title: 'flat', date: '2026-01-01' },
    ]);
