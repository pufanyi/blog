...
test('contents directories are prerendered with immediate children and publication dates', async ({
  ...
  for (const slug of [
    ...
    try {
      ...
      for (const [child, dates] of children) {
        const entryPath = POSTS.some((post) => post.slug === child)
          ? `/blog/${child}`
          : `/blog/contents/${child}`;
      ...
      for (const child of children.keys()) {
        expect(text).toContain(
          POSTS.some((post) => post.slug === child)
            ? `/blog/${child}.md`
            : `/blog/contents/${child}/index.md`,
        );
      }
    } finally {
      dom.window.close();
  ...
  const missing = await request.get('/blog/oi-icpc/codeforces/missing.md', {
...
test('an HTTP-only reader can discover and fetch the complete Markdown collection', async ({
  request,
}) => {
  ...
  expect(home.headers()['link']).toContain(
    '<https://pufanyi.com/profile.md>; rel="alternate"; type="text/markdown"',
  );
  expect(home.headers()['link']).toContain('<https://pufanyi.com/llms.txt>; rel="describedby"');
  expect(html).toMatch(
    /<link[^>]+rel="alternate"[^>]+type="text\/markdown"[^>]+href="https:\/\/pufanyi.com\/profile.md"/,
  );
  ...
  const paths = [...markdown.matchAll(/https:\/\/pufanyi.com(\/blog\/[^)\s]+\.md)/g)].map(
    (match) => match[1],
  );
  ...
  for (const path of paths) {
    ...
    expect(response.headers()['link'], path).toContain(
      `<https://pufanyi.com${path.slice(0, -3)}>; rel="canonical"`,
    );
...
test('profile identity, feeds and authored updates are discoverable in prerendered content', async ({
  request,
}) => {
  ...
  const sitemap = new JSDOM(await (await request.get('/sitemap.xml')).text(), {
  ...
  try {
    ...
    for (const post of POSTS) {
      ...
      expect(entry.querySelector('updated')?.textContent).toBe(
      ...
      const sitemapEntry = [...sitemap.window.document.querySelectorAll('url')].find(
        (item) => item.querySelector('loc')?.textContent === canonical,
      )!;
    ...
    for (const post of POSTS.filter((post) => post.updated)) {
      ...
      try {
        ...
        expect(
          document.querySelector(`app-post-header time[datetime="${post.updated}"]`),
        ).not.toBeNull();
...
test('Cloudflare serves sitemap canonicals directly and redirects HTML slash variants consistently', async ({
  request,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== 'cloudflare',
    'Cloudflare controls static HTML URL normalization',
  );
  const dom = new JSDOM(await (await request.get('/sitemap.xml')).text(), {
    contentType: 'application/xml',
  });
  try {
  ...
  for (const path of [
    ...
    '/blog',
    '/blog/contents',
    '/blog/contents/oi-icpc',
    '/blog/page/2',
    '/blog/oi-icpc/codeforces/cf77c',
    '/icpc',
  ]) {
    const response = await request.get(`${path}/?ref=search`, {
      maxRedirects: 0,
      headers: { Accept: 'text/html' },
    });
...
test('original URLs negotiate Markdown while browsers, HEAD requests and 404s keep correct behavior', async ({
  request,
}, testInfo) => {
  for (const [path, alternate] of [
    ...
    for (const accept of [
      ...
      expect(response.headers()['link']).toContain(
        `<https://pufanyi.com${alternate}>; rel="alternate"`,
      );
      if (markdown) {
  ...
  for (const path of [
    '/blog/nonexistent-export',
    '/blog/page/999',
    '/blog/nonexistent-export.md',
  ]) {
  ...
  if (testInfo.project.name === 'cloudflare') {
    const html = await request.get('/blog/oi-icpc/codeforces/cf77c', {
      headers: { Accept: 'text/html' },
    });
    const markdown = await request.get('/blog/oi-icpc/codeforces/cf77c', {
      headers: { Accept: 'text/markdown' },
    });
    ...
    for (const [accept, previousTag, status] of [
      ...
      if (status === 200) {
        expect(await response.text()).toBe(
          await (accept === 'text/markdown' ? markdown : html).text(),
        );
      }
    }
  }
...
test('published Markdown keeps technical content and HTML advertises the corresponding article', async ({
  request,
}) => {
  const html = await (await request.get('/blog/oi-icpc/codeforces/cf77c')).text();
  expect(html).toMatch(
    /<link[^>]+rel="alternate"[^>]+href="https:\/\/pufanyi.com\/blog\/oi-icpc\/codeforces\/cf77c.md"/,
  );
  ...
  const contest = await (
    await request.get('/blog/oi-icpc/other-problems/mock-contest-20190307.md')
  ).text();
  expect(contest).toContain(
